...
void QuadraticProbingTable::rehash() {
	// 0. Ñîõðàíÿåì ñòàðûé ìàññèâ cells â tempCells
	// 1. Èçìåíÿåì âíóòðåííåå ñîñòîÿíèå:
	// èçìåíÿåì capacity
	// âûäåëÿåì ïàìÿòü ïîä ÿ÷åéêè â ñîîòâåòñòâèè ñ capacity (ìàññèâ cells)
	// 2. Ïåðåíîñèì ÿ÷åéêè èç ñòàðîé òàáëèöû â íîâóþ:
	// 3. Îñâîáîäèòü ïàìÿòü, çàíèìàåìóþ ìàññèâîì tempCells.
...
void QuadraticProbingTable::insert(TValue value) {
	...
	while (cells[index].state == BUSY) {
		index = (index + (k + k * k) / 2) % capacity; // òðåóãîëüíûå ÷èñëà, ïðîõîä ïî âñåì åñëè capacity = ñòåïåíü äâîéêè
		k++;
	}
